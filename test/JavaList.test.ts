...
import { JavaCollection } from "../src/collections/JavaCollection.js";
...
describe("JavaList basics", () => {
  it("is a JavaCollection", () => {
    assert.ok(new JavaList<number>() instanceof JavaCollection);
  });

  it("builds from anything iterable", () => {
    assert.deepEqual(new JavaList<number>([1, 2, 3]).toArray(), [1, 2, 3]);
    assert.deepEqual(new JavaList<number>(new Set([1, 2])).toArray(), [1, 2]);
    assert.deepEqual(new JavaList<number>(new JavaSet<number>([4, 5])).toArray(), [4, 5]);
    assert.deepEqual(new JavaList<number>().toArray(), []);
  });

  it("keeps duplicates, unlike a set", () => {
    assert.equal(new JavaList<number>([1, 1, 1]).size(), 3);
  });

  it("appends with add, always reporting true", () => {
    const list = new JavaList<number>();
  ...
  it("reports size and emptiness", () => {
    assert.equal(new JavaList<number>().isEmpty(), true);
    assert.equal(new JavaList<number>([1]).isEmpty(), false);
    assert.equal(new JavaList<number>([1, 2]).size(), 2);
  });
});

describe("JavaList index operations", () => {
  it("gets by index", () => {
  ...
  it("throws IndexOutOfBoundsException rather than yielding undefined", () => {
    ...
    // anyone notices
    const list = new JavaList<string>(["a"]);
  ...
  it("names the offending index in the message", () => {
    assert.throws(() => new JavaList<string>(["a"]).get(7), { message: "Index 7 out of bounds for length 1" });
  });
  ...
  it("separates removal by index from removal by value", () => {
    ...
    const list = new JavaList<number>([10, 20, 30]);
...
describe("JavaList equality-based search", () => {
  it("finds a structurally equal element, where Array.indexOf would not", () => {
    const list = new JavaList<Point>([new Point(1, 2), new Point(3, 4)]);
  ...
  it("returns -1 for an absent element", () => {
    assert.equal(new JavaList<Point>([new Point(1, 2)]).indexOf(new Point(9, 9)), -1);
  });

  it("distinguishes indexOf from lastIndexOf", () => {
    const list = new JavaList<string>(["a", "b", "a"]);
  ...
  it("reports false when removing something absent", () => {
    assert.equal(new JavaList<string>(["a"]).remove("z"), false);
  });

  it("handles null and NaN elements", () => {
    const list = new JavaList<any>([null, NaN, 0]);
...
describe("JavaList bulk operations", () => {
  ...
  it("clears", () => {
    const list = new JavaList<number>([1, 2]);
    list.clear();
  ...
  it("sorts in place with the comparator it is given", () => {
    const list = new JavaList<number>([3, 1, 2]);
    list.sort((a, b) => a - b);
...
describe("JavaList.equals and hashCode", () => {
  it("is order-sensitive, unlike a set", () => {
    assert.equal(new JavaList<number>([1, 2]).equals(new JavaList<number>([1, 2])), true);
    assert.equal(new JavaList<number>([1, 2]).equals(new JavaList<number>([2, 1])), false);
    assert.notEqual(new JavaList<number>([1, 2]).hashCode(), new JavaList<number>([2, 1]).hashCode());
  });

  it("agrees with hashCode for equal lists", () => {
    assert.equal(new JavaList<Point>([new Point(1, 2)]).hashCode(), new JavaList<Point>([new Point(1, 2)]).hashCode());
  });

  it("compares elements with equals", () => {
    assert.equal(new JavaList<Point>([new Point(1, 2)]).equals(new JavaList<Point>([new Point(1, 2)])), true);
  });

  it("is unequal on differing length", () => {
    assert.equal(new JavaList<number>([1]).equals(new JavaList<number>([1, 2])), false);
  });

  it("is reflexive, and empty lists are equal", () => {
    const list = new JavaList<number>();
    assert.equal(list.equals(list), true);
    assert.equal(list.equals(new JavaList<number>()), true);
    assert.equal(list.hashCode(), 1, "Java's AbstractList seeds the hash at 1");
  });

  it("is not equal to a set holding the same elements", () => {
    assert.equal(new JavaList<number>([1, 2]).equals(new JavaSet<number>([1, 2])), false);
  });

  it("returns false, and does not throw, for junk arguments", () => {
    const list = new JavaList<number>([1]);
    ...
    const cases: [string, unknown][] = [
      ...
      ["plain object", {}],
      ["array", [1]],
      ["prototype-only forgery", Object.create(JavaList.prototype)],
    ];
...
describe("JavaList.toString", () => {
  it("matches Java's AbstractCollection format", () => {
    assert.equal(new JavaList<number>().toString(), "[]");
    assert.equal(new JavaList<number>([1, 2, 3]).toString(), "[1, 2, 3]");
    assert.equal(new JavaList<Point>([new Point(1, 2)]).toString(), "[(1,2)]");
  });
});

describe("JavaList immutability", () => {
  it("JavaList.of refuses every mutator", () => {
    const list = JavaList.of(1, 2, 3);
    assert.deepEqual(list.toArray(), [1, 2, 3]);
  ...
  it("unmodifiable is a live view, not a copy", () => {
    const base = new JavaList<number>([1]);
    const view = JavaList.unmodifiable(base);
    base.add(2);
  ...
  it("JavaList.of is a frozen copy, with no original to track", () => {
    const source = [1, 2];
    const list = JavaList.of(...source);
    source.push(3);
