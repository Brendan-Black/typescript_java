 ...
 * ```
 *
 * Java has kept both this and `List.sort` since Java 8 — this one delegates to {@link JavaList.sort}, exactly as
 ...
 *   in common
 */
export function sort<T>(list: JavaList<T>, comparator: (a: T, b: T) => number): void;
export function sort<T extends NaturallyOrdered>(list: JavaList<T>): void;
export function sort<T>(list: JavaList<T>, comparator?: (a: T, b: T) => number): void {
...
function extreme<T>(values: Iterable<T>, comparator: (a: T, b: T) => number, keep: 1 | -1, operation: string): T {
  // snapshotting, as the bulk operations on JavaCollection do — see the note on that class
  const items = [...values];
  if (items.length === 0) {
 ...
 */
export function binarySearch<T>(list: JavaList<T>, key: T, comparator: (a: T, b: T) => number): number;
export function binarySearch<T extends NaturallyOrdered>(list: JavaList<T>, key: T): number;
export function binarySearch<T>(list: JavaList<T>, key: T, comparator?: (a: T, b: T) => number): number {
...
export function reverse<T>(list: JavaList<T>): void {
 ...
 *
 * Written as Java writes it, and the nesting is the point rather than showing off: {@link JavaList.set} returns
 * the element it displaced, so the inner call both stores and yields the value the outer call needs.
 *
 * @throws IndexOutOfBoundsException if either index is out of range
 */
export function swap<T>(list: JavaList<T>, i: number, j: number): void {
