 ...
 *
 * Three of these carry an `Of` suffix at the top level — `hashCodeOf`, `equalsOf`, `compareOf` — and one is
 * spelled `hashAll`. Those names exist only because a module cannot export a bare `hashCode` or `equals`
 * without the reader having to guess what they belong to. Inside a namespace the qualifier does that job, so
 * they take the names Java gives them and `Java.Objects.hash(x, y)` reads as the `Objects.hash(x, y)` it is.
 *
 * @module
 */
...
export {
  /** Java's `Objects.equals(a, b)`. Exported flat as `equalsOf`. */
  equalsOf as equals,
  /** Java's `Objects.hash(...)`. Exported flat as `hashAll`. */
  hashAll as hash,
  /** Java's `Objects.hashCode(o)`. Exported flat as `hashCodeOf`. */
...
export {
   ...
   * relational operators. Exported flat as `compareOf`.
   *
